 ...
 * 2. 预置背景：
 *    - 首选「完整清晰图」的 dataURL（saveDisplayedImage 双写到 localStorage 的
 *      wp:display）。它是同步可读的，于是首帧直接画出真正的清晰壁纸，
 *      整张图随首帧一并上屏——「打开即见高清」，无需等 React 挂载、
 *      无需走 IndexedDB、无需「先糊后清」。
 *    - 该 dataURL 尚不存在（首次安装 / 图库被清 / 隐私模式）时，退化到 32px
 *      模糊兜底（wp:backdrop），观感 = 「模糊壁纸 → 清晰壁纸」，仍无白屏。
 *
  ...
  try {
    ...
    var base =
      /^\d{1,3},\d{1,3},\d{1,3}$/.test(color || "")
        ? "rgb(" + color + ")"
        : themeBase;
    // 首帧优先画「完整清晰图」；没有才回退到 32px 模糊兜底
    var full = localStorage.getItem("wp:display");
    var bg = full || localStorage.getItem("wp:backdrop") || "";
